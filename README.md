...
place-based contextual data for multilevel health research,
implementation science, and population health analytics.

The package supports address geocoding, Census tract identification,
end-to-end record linkage, and enrichment with hosted Cancer InFocus
contextual data (<https://cancerinfocus.org>). In the typical workflow,
users provide individual-level records, ContextLinkr identifies Census
tract GEOIDs when needed, and contextual measures are retrieved from
Cancer InFocus rather than supplied manually by the user.

ContextLinkr also includes lower-level helpers for joining user-supplied
contextual datasets, but the primary end-user workflow is designed
around Cancer InFocus context retrieval, cache controls, source
metadata, and output provenance.

## Installation

ContextLinkr is currently in collaborator-beta development and can be
